...
export default function NewsPage() {
    ...
    useEffect(() => {
        ...
        // Find by number ID or fallback string
        const item = newsArticles.find(n => n.id.toString() === params.id.toString());

        if (item) {
            setNews(item);
        } else {
            // Give it a small delay to handle hydration if needed
            const timer = setTimeout(() => {
                const retryItem = newsArticles.find(n => n.id.toString() === params.id.toString());
                if (retryItem) setNews(retryItem);
                else router.push('/');
            }, 100);
